...
export const useCustomers = (params?: {
  ...
  return useQuery({
    ...
    queryFn: () => customersApi.getCustomers(queryParams),
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
...
export const useCreateCustomer = () => {
  ...
  return useMutation({
    mutationFn: (customerData: Omit<Customer, 'id' | 'createdAt' | 'updatedAt'>) =>
      customersApi.createCustomer({ ...customerData, organizationId: organizationId! }),
    ...
    onSuccess: (newCustomer, variables, context) => {
      ...
      const createdCustomer = (newCustomer as { data: Customer })?.data || (newCustomer as unknown as Customer);
